  ...
  <div class="message-item">
    <div class="sys-info">
      <div :class="[
        ...
        Video 【{{ data.videoName }}】audit 
        {{ data.extendDto.auditStatus == 3 ? "success" : "fail" }}
      </div>
      <div class="resean" v-if="data.extendDto.auditStatus != 3 ">
        fail reason:{{ data.extendDto.messageContent }}
      </div>
      <div class="success" v-if="data.extendDto.auditStatus == 3">
        <router-link class="a-link" :to="`/video/${data.videoId}`" target="_blank">Details</router-link>
      </div>
      <div class="send-time">
