<template>
  <div class="video-panel">
    <div class="video-title-panel">
      <div class="video-title">My Collection</div>
    </div>
    <div v-if="dataSource.list && dataSource.list.length == 0">
      <NoData msg="You have not collect yet~~"></NoData>
    </div>
    <DataGridList :dataSource="dataSource" v-else @loadData="loadVideoList">
      <template #default="{ data }">
        <div class="data-item">
          ...
          <div class="video-name" @click="jump(data)">{{ data.videoName||"Failed video" }}</div>
          <div class="collection-info">
            <div class="collection-time">
              collect at： {{ proxy.Utils.formatDate(data.actionTime) }}
            </div>
            <el-dropdown>
              <div class="iconfont icon-more" @click.stop v-show="myself"></div>
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item @click.stop="cancelCollection(data)">Cancel collection</el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </el-dropdown>
          </div>
...
const cancelCollection = (data) => {
  proxy.Confirm({
    message: 'Sure to cancel collection？',
    okfun: async () => {
      let result = await proxy.Request({
