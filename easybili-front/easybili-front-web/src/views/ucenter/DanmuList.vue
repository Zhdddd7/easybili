  ...
  <div class="danmu-panel">
    ...
    <Table ref="tableInfoRef" :columns="columns" :fetch="loadDataList" :dataSource="tableData" :options="tableOptions"
      ...
      <template #slotNickName="{index,row}">
        <router-link target="_blank" class="nick-name" :to="`/user/${row.userId}`">{{row.userName}}</router-link>
      </template>
      ...
      <template #slotOperation="{ index, row }">
        <a href="javascript:void(0)" class="a-link" @click="delDanmu(row.danmuId)">delete</a>
      </template>

      <template #slotText="{index,row}">
        <div>{{row.text}}</div>
        <router-link target="_blank" class="video-info" :to="`/video/${row.videoId}`">video：{{row.videoName}}</router-link>
      </template>
    </Table>
  </div>
</template>
