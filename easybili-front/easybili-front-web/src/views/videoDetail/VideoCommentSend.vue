    ...
    <div class="input-panel">
      <template v-if="showSend">
        <el-form :model="formData" :rules="rules" ref="formDataRef" @submit.prevent>
          ...
          <el-form-item label="" class="input-area">
            <el-input id="inputPanel" ref="inputRef" clearable
                      :placeholder="formData.userName ? '@' + formData.userName : ''" v-model="formData.content" :rows="1"
                      type="textarea" resize="none" :show-word-limit="true" :maxlength="500"
        ...
        <div class="op-panel">
          ...
          <div class="send-btn">
            <el-button type="primary" @mousedown="submitComment">Send</el-button>
          </div>
        </div>
      </template>
      <div v-else class="no-send">
        The up has shut the comment
      </div>
    </div>
  </div>
</template>
...
const rules = {
  content: [{required: true, message: 'Entering...'}],
}
