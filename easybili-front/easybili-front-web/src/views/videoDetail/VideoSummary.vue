<template>
  <div class="summary-panel">
    <div class="summary" :style="{ height: expandType ? 'auto' : '90px' }">
      <div class="summary-inner" id="summary-inner" v-html="videoInfo.introduction"></div>
    </div>
    <div class="expand-btn" @click="expand" v-show="showExpandBtn">
      {{ expandType ? "Show less" : "Show more" }}
    </div>
    <div class="tag-list">
      <router-link :to="`/search?keyword=${item}`" class="tag-item" target="_blank"
        v-for="item in videoInfo.tags">{{ item }}</router-link>
    </div>
  </div>
</template>
